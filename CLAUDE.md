...
Received from the organiser during the exercise (the source emails and the trast.de screenshots were in `ref/`, removed during the cleanup; their substance is kept here and in `docs/01-what-was-built.md`):

- **Assigned client: trast digital gmbh** (<https://trast.de/>). Target users: *parts receiving leads and colleagues reconciling delivery and invoice evidence*. **Client fit is a review criterion** — how closely the UI, visual language, wording and workflow reflect the assigned organisation.
- What was read off trast.de and adopted: all-lowercase headings and nav, pill-outline buttons, a geometric sans (Montserrat), indigo/electric-blue/coral on light grey, and an informal German voice (*du*). The collision that had to be resolved: indigo was this prototype's "simulated" colour **and** trast's brand colour — the brand took indigo, simulated moved to coral.
- The organiser also asked for a **shareable clickable preview** ahead of further feature work, which is why the app is deployed at <https://dail-c04.vercel.app>.

Our own writing:

- `README.md` — the deliverable README: result first, live link, run instructions, real vs simulated, limitations, next validation test. Keep it short; the detail lives in `docs/`.
- `docs/` — the documentation set for the organiser. `01-what-was-built.md` (feature by feature), `02-how-it-works.md` (data model, engine, where writes are allowed), `03-real-vs-simulated.md` (the honest split with evidence), `04-wolf-handoff.md` (the filled handoff), `05-demo-script.md` (the talk), `demo-meme.jpeg`. These restate claims the demo must be able to back — change them with the feature list, not after it.
...
`web/.env.local` holds the Supabase URL and the secret key and is gitignored; `web/.env.example` documents the shape. There is no publishable key: nothing runs against Supabase from the browser.
...
- `src/lib/supabase/server.ts` — the **only** Supabase client, holding the secret key and marked `server-only` so it can never be bundled into a browser chunk. `queries.ts` (reads) and `actions.ts` (writes) both go through it; nothing talks to Supabase from the browser, and no key is shipped to it. There is deliberately no browser client — one existed, was never imported, and was removed.
- RLS grants select only. That is the **backstop**, not the primary guard: a write from a browser key would be refused (`42501`), but the app never creates one. The primary guard is that every write lives in `actions.ts` behind review.
